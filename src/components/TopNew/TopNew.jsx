...
import { FaBell, FaCog, FaSignOutAlt, FaCalendarAlt } from "react-icons/fa"; // Added calendar icon
...
const TopNew = () => {
  ...
  const [showCalendar, setShowCalendar] = useState(false); // State for calendar overlay
  ...
  return (
    <header className="topbar">
      ...
      <div className={`topbar-container ${showSearch ? "search-active" : ""}`}>
        {showSearch ? (
          ...
          <div className="top-icons">
            ...
            <FaCalendarAlt className="calendar-icon" onClick={toggleCalendar} style={{ cursor: "pointer", marginRight: "15px" }} /> {/* Calendar icon */}
            <FaBell
      ...
      {showCalendar && <Calendar onClose={toggleCalendar} />} {/* Render calendar when state is true */}
    </header>
  );
};
